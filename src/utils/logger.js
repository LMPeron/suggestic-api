...
module.exports = class Logger {
    ...
    if (args && args.length > 0) {
      for (let i = 0; i < args.length; i += 1) {
        if (args[i] && typeof args[i] === "object") {
          ...
          str +=
            stringifiedObject === "{}" ? args[i].toString() : stringifiedObject;
        } else if (args[i]) {
          str += ` ${args[i]}`;
        }
      }
  ...
  getLogger() {
    ...
    const self = this;
    if (global.conf && global.conf.logLevel)
      closureLogger.level = global.conf.logLevel;
    else closureLogger.level = "DEBUG";
  ...
  convertToString(data) {
    try {
      ...
      log4js
        .getLogger("logger")
        .error("Error occured during converting JSON to string:", err);
      return data;
    }
  }
  ...
  convertToJson(data) {
    try {
      ...
      log4js
        .getLogger("logger")
        .error("Error occured during converting data to JSON:", err);
      return data;
    }
  }
};
